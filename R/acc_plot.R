...
# Arguments:
# `auc_values`: a data.frame returned from compute_aucs()
# `model.name`: a string contains the name of the model
# that was used to obtain `auc_values`
acc_plot <- function(auc_values, model_name) {
  ...
  plot <- ggplot2::ggplot(
    ...
    ggplot2::scale_x_reverse(labels = function(x) {
      glue::glue("{x}\n({round(x / max(auc_values$subset.size) * 100, 1)}%)")
    }) +
